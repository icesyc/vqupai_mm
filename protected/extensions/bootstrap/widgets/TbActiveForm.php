...
class TbActiveForm extends CActiveForm
{
	...
	//生成一个buttonGroup类型的radio
	public function buttonGroup($model, $attribute, $data){
		$id = CHtml::getIdByName(CHtml::resolveName($model, $attribute));
		Yii::app()->clientScript->registerScript('buttonGroup_' . $id, "
		    $('#{$id}_btg.btn-group a').click(function(){
		        var value = $(this).data('value');
		        $('#{$id}').val(value);
		    });
		", CClientScript::POS_END);

		$current = $model->$attribute === null ? array_shift(array_keys($data)) : $model->$attribute;
		$switchs = array();
		foreach($data as $value => $text){
			$switchs[] = array(
				'label' => $text,
				'htmlOptions' => array('data-value' => $value),
				'active' => strcmp($current, $value) == 0
			);
		}
		ob_start();
		...
		$this->widget('bootstrap.widgets.TbButtonGroup', array(
			'toggle' => 'radio', 
			'buttons' => $switchs,
			'htmlOptions' => array('id' => $id . '_btg')
		));
		echo CHtml::activeHiddenField($model, $attribute, array('value' => $current));
		echo '</p>';
		$output = ob_get_clean();
		return $output;
	}
